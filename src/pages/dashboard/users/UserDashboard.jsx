...
const UserDashboard = () => {
  ...
  return (
    <div className=" bg-gray-100 !py-16">
      <div style={{ marginLeft: "auto", marginRight: "auto" }} className="max-w-4xl  bg-white shadow-md rounded-lg !p-6">
            ...
            <ul className="space-y-4">
              {orders.map((order) => (
                ...
                >
                  ...
                  <p className="font-bold">Date: {new Date(order?.createdAt).toLocaleDateString()}</p>
                  <p className="font-bold">Total: ${order.totalPrice}</p>
                  {order.productIds.map((productId) => (
                    <p key={productId} className="!ml-1 !mt-2">
                      {productId}
                    </p>
                  ))}
                </li>
